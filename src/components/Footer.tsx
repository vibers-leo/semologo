...
import { T, useLocale } from "@/lib/locale-context";
      ...
      <div
          ...
          <div style={{ borderBottom: "1px solid #1f1f23", padding: "32px 24px 24px", maxWidth: 1280, margin: "0 auto" }}>
            <div style={{ display: "grid", gridTemplateColumns: "1fr auto", gap: 32, alignItems: "start" }}>
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
                <div style={{ fontSize: 16, fontWeight: 800, color: "#ffffff", letterSpacing: "-.02em" }}><T>{"세모로고"}</T></div>
                ...
                <div style={{ display: "flex", gap: 12, fontSize: 11 }}>
                  <Link href="/terms" style={{ color: "#71717a", textDecoration: "none" }}><T>{"이용약관"}</T></Link>
                  <span style={{ color: "#3f3f46" }}>|</span>
                  <Link href="/privacy" style={{ color: "#71717a", textDecoration: "none" }}><T>{"개인정보처리방침"}</T></Link>
                </div>
                <div>
                  <button
                    onClick={() => setBizOpen(o => !o)}
                    style={{ display: "flex", alignItems: "center", gap: 4, fontSize: 11, color: "#52525b", background: "none", border: "none", cursor: "pointer", padding: 0 }}><T>{"사업자 정보"}</T><svg width="9" height="9" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5"
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
                <div style={{ fontSize: 11, fontWeight: 700, color: "#ffffff", marginBottom: 4 }}><T>{"메뉴"}</T></div>
                {[
                  ...
                  href.startsWith("mailto") ? (
                    ...
                    <Link key={t(label)} href={href} style={{ fontSize: 11, color: "#71717a", textDecoration: "none", whiteSpace: "nowrap" }}>{t(label)}</Link>
                  )
                )}
              </div>
            </div>
        ...
        <div style={{ maxWidth: 1280, margin: "0 auto", padding: "8px 24px", minHeight: 44, display: "flex", flexWrap: "wrap", rowGap: 6, columnGap: 12, alignItems: "center", justifyContent: "space-between" }}>
          <span style={{ fontSize: 12, color: "#52525b", whiteSpace: "nowrap" }}>
            © {YEAR}{" "}<T>{"주식회사 계발자들 · 세모로고"}</T></span>
          <div style={{ display: "flex", alignItems: "center", gap: 16, flexWrap: "wrap", rowGap: 6 }}>
            <Link href="/submit" style={{ fontSize: 12, color: "#71717a", textDecoration: "none", whiteSpace: "nowrap" }}><T>{"로고 제보"}</T></Link>
            <Link href="/request" style={{ fontSize: 12, color: "#71717a", textDecoration: "none", whiteSpace: "nowrap" }}><T>{"로고 요청"}</T></Link>
            <button
              onClick={() => setOpen(o => !o)}
